...
public class Noise {
    ...
    public static double[][] noiseLayer(int p, int w, int h)
    {
        ...
        public static double[][] noiseLayerSmallRegion(int p, int w, int h)
    {
        double[][] output = new double[h][w];
        WeightVector[][] vectorArr = new WeightVector[(h/p)+2][(w/p)+2];
            //
            ...
            for (int i = 0; i < vectorArr.length; i++)
            {
                ...
                {
                    // each vector in vectorArr gets a random x and y value
                    int randInt = (int)(Math.pow(Math.random(), 4)*8);
                    vectorArr[i][j] = new WeightVector(randomVectors[randInt][0],randomVectors[randInt][1]);
    ...
    public static int[][] octaveNoiseLayerSmallRegion(int w,int h)
    {
        double[][] noise64 = noiseLayerSmallRegion(64,w,h);
        double[][] noise32 = noiseLayerSmallRegion(32,w,h);
        double[][] noise16 = noiseLayerSmallRegion(16,w,h);
        double[][] noise8 = noiseLayerSmallRegion(8,w,h);
        int[][] output = new int[h][w];
